...
import {
  InputFilmsProps, ClickFilmsProps, FetchedButtonProp, FilmsCollection,
} from '../interfaces';

export let parametrMovies: any; // BEDA
function getParamsMovies(titleMovie:string, paramMovies:string): object {
  return {
    titleMovie,
    paramMovies,
  };
...
const FilmsForm: React.FC <FilmsCollection> = ({ }) => {
  ...
  const changeHandler = ({ target }: React.ChangeEvent<HTMLFormElement>): void => {
    setTitle(target.value);
  };

  const clickHandler = (paramReq: string): void => {
    setParamMovies(paramReq);
  };

  async function submitHandleKeyPress(event: React.KeyboardEvent<HTMLFormElement>): Promise <object> {
    if (event.key === 'Enter') {
  ...
  async function submitHandler(event: React.ChangeEvent<HTMLFormElement>): Promise <object> {
