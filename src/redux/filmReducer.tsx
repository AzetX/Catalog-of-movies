import {
  FETCH_FILMS, SORTING_FILMS_YEAR, FETCH_FILM_FULL, GET_MOVIES_WTH_FAILURE, GET_MOVIE_WTH_FAILURE, SORTING_FILMS_TITLE_STRAIGHT, SORTING_FILMS_TITLE_REVERSE,
} from './types';
import { StateMovies, StateMovie } from '../interfaces';

export const initialStateMovies: StateMovies = {
  fetchedFilms: [],
...
export const MoviesRequestReducer = (state = initialStateMovies, action: any) => {
  switch (action.type) {
    ...
    case SORTING_FILMS_YEAR:
      return {
        ...state,
        fetchedFilms: state.fetchedFilms.map((item, index, arr) => action.payload[index]).sort((a, b) => {
          if (a.Year > b.Year) {
            return 1;
          }
          if (a.Year < b.Year) {
...
const initialStateMovie: StateMovie = {
  fetchedFullMovie: [],
  incorrectRequestMovie: null,
};
